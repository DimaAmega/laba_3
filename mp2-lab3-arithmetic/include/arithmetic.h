	...
	int i = 0;
		...
		switch (getPriority(i)) 
		{
			...
			{              if (i < strlen(onestring) - 1)
				...
				else a.push(1); else { error = true; cout << "error on the " << i+1 << "position";
				}
	 break;
 }
			case 2:
			{
				if (i > 0) if (onestring[i - 1] == '.') { error = true; cout << "error on the " << i << "position";
				}
				if (i < strlen(onestring) - 1)
					if ((getPriority(i + 1) >= 1)||(getPriority(i + 1) == -1) || (onestring[i + 1] == '.')) { error = true; cout << "error on the " << i+2 << "position";
				}
					else a.push(2); else { error = true; cout << "error on the " << i +1 << "position";
					}
				break;
			}
			...
			case -2:
			{
				if (i > 0) if (getPriority(i - 1) == -3) if (!(onestring[i - 1] == '.')) { error = true; cout << "error on the " << i << "position";
				}
				else a.push(-2);
				else a.push(-2);
			...
			case -3:
			{
				...
				if (i > 0) if ((getPriority(i - 1) == -2)|| (onestring[i - 1] == '.')|| (onestring[i + 1] == '.')) { error = true; cout << "error on the " << i+2 << "position";
				}
				else a.push(-2);
				break;
			}
